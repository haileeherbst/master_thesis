...
threshold_hybrids_MJ <- readRDS(here::here("data/stringent_threshold_MJ.rds"))
...
MJ_hyb <- qCLASH_untreated %>% filter(grepl("MelJuso", cell_line))
...
IPC_thresh_hyb <- threshold_hybrids_MJ %>% filter(grepl("^IPC$", cell_line))
SKMel_thresh_hyb <- threshold_hybrids_MJ %>% filter(grepl("^SK$", cell_line))
MJ_thresh_hyb <- threshold_hybrids_MJ %>% filter(grepl("^MJ$", cell_line))
IPC_R_thresh_hyb <- threshold_hybrids_MJ %>% filter(grepl("IPC_R", cell_line))
SKMel_R_thresh_hyb <- threshold_hybrids_MJ %>% filter(grepl("SK_R", cell_line))
...
MJ <- semi_join(MJ_hyb, MJ_thresh_hyb, by = "hybrids")
```


# Create one dataframe of all 
```{r}
threshold_qCLASH <- bind_rows(SK_R, SK, IPC_R, IPC, MJ)
saveRDS(threshold_qCLASH, "threshold_qCLASH_MJ.RDS")
```
...
unique_hybrids <- as.data.frame(unique(threshold_hybrids_MJ$hybrids, incomparables = FALSE))
names(unique_hybrids)[names(unique_hybrids) == 'unique(threshold_hybrids_MJ$hybrids, incomparables = FALSE)'] <- 'hybrids'
...
MJ_count <- hybrid_count_table[,c(1, 14:16)]
MJ_count <- semi_join(MJ_count, MJ_thresh_hyb, by = "hybrids")
...
count_table_threshold <- left_join(count_table_threshold, MJ_count, by = "hybrids")

saveRDS(count_table_threshold, "count_table_threshold_MJ.RDS")
```
